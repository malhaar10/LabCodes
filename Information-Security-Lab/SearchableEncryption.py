...
def create_index(documents, key):
    ...
    for word_hash, doc_ids in index.items():
        encrypted_doc_ids = [
            encrypt_data(key, str(doc_id).encode())[1] for doc_id in doc_ids
        ]
        encrypted_index[encrypt_data(key, word_hash)[1]] = encrypted_doc_ids
...
def search(encrypted_index, query, key):
    ...
    print("Encrypted Query Hash:", encrypted_query_hash)  # Debug print
    print("Encrypted Index:", encrypted_index)  # Debug print

    if encrypted_query_hash in encrypted_index:
        return [
            decrypt_data(key, *encrypt_data(key, doc_id))[0].decode() for doc_id in encrypted_index[encrypted_query_hash]
        ]
    else:
        return []
